...
{    
    ...
    while(!goal_poses.empty()){
        ...
        new_pose2d = search_2d(map,x_size,y_size,collision_thresh,robotposeX,robotposeY,curr_time,goalposeX,goalposeY,targetposeV,num_obj,object_traj,obj_size,target_steps);
        ...
        i=robotposeX, j=robotposeY, k=curr_time;
        cell c = {};
        ...
        while (!open.empty())
        {
            ...
            if (i==goalposeX && j==goalposeY && curr_time+k+time_elapsed<=target_steps)                                       // if goal pose is expanded
            {
                ...
                new_pose= getPath(grid, goalposeX, goalposeY, k);
                found_path=true;
            }
            ...
            for(int dir = 0; dir < NUMOFDIRS; dir++)
            {
                ...
                {                      
                    ...
                    if( (closed.find({newx,newy,newt}) == closed.end() || closed[{newx,newy,newt}]==false) && isValid(newx,newy,x_size,y_size,map,collision_thresh) && !collCheck(object_traj,num_obj,newx,newy,num_obj,obj_size,curr_time+k+time_elapsed,target_steps)) 
                    {
        ...
        if(!found_path){
            new_pose = search_2d(map,x_size,y_size,collision_thresh,robotposeX,robotposeY,curr_time,goalposeX,goalposeY,targetposeV,num_obj,object_traj,obj_size,target_steps);
            found_path=true;
            better_2dpath=true;
        }
        if(found_path==true){
